...
typedef struct {
    ...
    unsigned int bits[3];  //size of an unsigned int is 4 bytes
...
bitSet *initBitSet(int totalBits)
{
    ...
    ret = (bitSet *) malloc(sizeof(bitSet));
    ret->size = totalBits;
    return ret; //address of the start of the structure
}
...
int inc;
bitSet try[32];  //The size of the array is the number of tries.
bitSet* point=try;
